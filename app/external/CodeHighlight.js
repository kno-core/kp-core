/* Rainbow v2.1.4 rainbowco.de | included languages: css, generic, html, javascript, json */
!function (e, t) {
	...
	function p(e, t) {
		for (var r = {c: 0}, a = 0, o = e; a < o.length; a += 1) {
			...
			if (!i.classList.contains("rainbow") && s) {
				...
				var u = i.getAttribute("data-global-class"), c = parseInt(i.getAttribute("data-delay"), 10);console.log('radix',c);
...
var
	strReg1 = /"(.*?)"/g,
	strReg2 = /'(.*?)'/g,
	specialReg = /\b(new|var|let|const|if|do|function|Class|interface|export|while|switch|for|foreach|in|continue|break)(?=[^\w])/g,
	specialJsGlobReg = /\b(document|console|window|Array|String|Object|Number|\$)(?=[^\w])/g,
	specialJsReg = /\b(getElementsBy(TagName|ClassName|Name)|getElementById|log|typeof|instanceof|querySelector|querySelectorAll)(?=[^\w])/g,
	specialMethReg = /\b(indexOf|match|replace|toString|length|addRoute|enqueueBody|enqueueScript|enqueueStyle|enqueueHead|resolve|reject)(?=[^\w])/g,
	specialPhpReg = /\b(define|echo|print_r|var_dump)(?=[^\w])/g,
	specialCommentReg = /(\/\*.*\*\/)/g,
	inlineCommentReg = /(\/\/.*)/g;
let htmlTagReg = /(&lt;[^\&]*&gt;)/g;
let sqlReg = /\b(CREATE|ALL|DATABASE|TABLE|GRANT|PRIVILEGES|IDENTIFIED|FLUSH|SELECT|UPDATE|DELETE|INSERT|FROM|WHERE|ORDER|BY|GROUP|LIMIT|INNER|OUTER|AS|ON|COUNT|CASE|TO|IF|WHEN|BETWEEN|AND|OR)(?=[^\w])/g;
	...
	Array.from(element.childNodes).forEach(function (node) {
		switch (node.nodeType) {
			...
			case Node.ELEMENT_NODE:
				textSegments = textSegments.concat(getTextSegments(node));
				break;
...
function format(item) {
	...
	const textSegments = getTextSegments(item);
	...
	Rainbow.color(string, item.getAttribute('data-language')||'javascript', function(highlightedCode) {
		item.innerHTML = highlightedCode;
		//restoreSelection(item, anchorIndex, focusIndex);
	});
}
...
for (let i = 0; i < codes.length; i++) {
	...
	item.addEventListener('input', function (e) {
		...
		//	e.preventDefault()
		///	}else{
		//format(document.getElementById(this.id+'format'));
		//	}
...
if (repo_new_save) {
	repo_new_save.onclick = function () {
		var xmlhttp = new XMLHttpRequest();   // new HttpRequest instance
		var theUrl = "/repo/post/";
		xmlhttp.open("POST", theUrl);
		xmlhttp.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
		let payload = {"setup": document.getElementById('code-input-setup').innerText};
		if (this.getAttribute('data-object')) {
			payload["_id"] = this.getAttribute('data-object');
		}
